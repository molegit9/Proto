...
# sqlite:///./security_logs.db 와 같은 형식에서 경로 추출
...
@contextmanager
def get_db():
    # Make sure directory exists if there is a path
...
def get_recent_logs(limit: int = 20) -> list[dict]:
    with get_db() as conn:
        ...
        # sqlite3.Row를 dict로 변환하고 bool 값을 제대로 처리
        result = []
        for row in rows:
            d = dict(row)
            if d['type'] == 'email':
